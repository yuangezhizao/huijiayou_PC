...
var Payment_MobileCHZ = {
    ...
    AfterGetPaymentParams: function (msg) {
        ...
        var payment = msg;
        if (Payment_MobileCHZ.PageType == g_const_PayPageType.Redirect) {
            window.location.replace(Payment_MobileCHZ.resultmessage);
        }
        else if (Payment_MobileCHZ.PageType == g_const_PayPageType.Capture) {
            var payment1 = JSON.parse(Payment_MobileCHZ.resultmessage);
            if (payment1.resultcode == 0) {
    ...
    ChangePayment: function (paygate, orderno, callback) {
        ...
        if (bank != null) {
            var objdata = {
                action: "changepayment_MobileCHZ",
                OrderNo: orderno,
                paygate: bank.Paygate,
                paygatetypeaccount: bank.paygatetypeaccount,
                paygatetype: bank.paygatetype
