...
function SetRegisterMesaageInfo(em_id, input_id, sMessage) {
    $("#" + em_id).html(sMessage);
    if (sMessage.length == 0) {
        $("#" + em_id).attr("class", "");
    }
    else if (sMessage.length > 11) {
        $("#" + em_id).attr("class", "e2");
        $("#" + input_id).addClass("curr");
    }
    else {
        $("#" + em_id).attr("class", "e1");
        $("#" + input_id).addClass("curr");
...
function SetLoginMesaageInfo(input_id, sMessage) {
    $("#message").show().html(sMessage);
    $("#" + input_id).addClass("curr");
    $("#" + input_id).parent().find("em").hide();
}
function SetLoginDefaultInfo(tag, msg) {
    $("#message").hide();
    $("#" + tag).parent().find("em").show();
    $("#" + tag).removeClass("curr");
}

function SetPhoneLoginMesaageInfo(tag, msg) {
    $("#tips").hide();
    $("#phoneMessage").html(msg).show();
    $("#" + tag).parent().find("em").hide();
    $("#" + tag).addClass("curr");
}
function SetPhoneLoginDefaultInfo(tag, msg) {
    $("#tips").show();
    $("#phoneMessage").hide();
    $("#" + tag).parent().find("em").show();
    $("#" + tag).removeClass("curr");
}
function ShowMesaagePop(sMessage) {
    $("#sp_msg").html("<span></span>" + sMessage);
    $("#messageShow").addClass("popall").show();
}
...
function ShowMesaageShort(sMessage) {
    $("#sp_msg").html("<span></span>" + sMessage);
    $("#messageShow").addClass("popall").show();
    setTimeout(function () {
        $("#messageShow").hide();
...
function ConfirmMessage(sMessage, callBackFail, callBackSuccess, textFail, textSuccess) {
    $("#confirm_sp_msg").html("<span></span>" + sMessage);
    $("#ConfirmMessage").show();
    ...
    if (textFail) {
        $("#btnCancel").html(textFail);
    }
    if (textSuccess) {
        $("#btnConfirm").html(textSuccess);
    }
}
    ...
    if (u && u.Trim().length > 0) {
        switch (t) {
            ...
            case g_const_showmoreLinktype.ProductDetail:
                sreturn = g_const_PageURL.GetLink(g_const_PageURL.ProductDetail, "&pid=" + u);// "/Product/ProductDetail.html?pid=" + u;
                break;
            case g_const_showmoreLinktype.ProductType:
                sreturn = g_const_PageURL.GetLink(g_const_PageURL.Product_List, "&showtype=category&keyword=" + u);//"/Product_List.html?showtype=category&keyword=" + u;
            case g_const_showmoreLinktype.KeyWordSearch:
                sreturn = g_const_PageURL.GetLink(g_const_PageURL.Product_List, "&keyword=" + encodeURIComponent(u));//"/Product_List.html?keyword=" + (u);
                break;
            default:
...
var PageUrlConfig = {
    ...
    //跳转页面并且设置返回页面 (目标页面，返回页面，参数)
    SetReturnGoto: function (objUrl, returnUrl, pageParam) {
        PageUrlConfig.SetUrl(g_const_PageURL.MainIndex + "?u=" + returnUrl + pageParam);
        var p = "&t=" + Math.random();
        g_index.GoTo(g_const_PageURL.GetKey(objUrl), p);
    },
    //传入步骤
    //传入步骤
    BackTo: function (num) {
...
if (GetQueryString("fromurl") != "") {
    ...
    localStorage[g_const_localStorage.OrderFromRefer] = GetQueryString("fromurl");
    //记录失效时间(30天后)
    var Expires_duomai = Date.parse(new Date()) + 30 * 24 * 60 * 60 * 1000;
    localStorage[g_const_localStorage.Expires_duomai] = Expires_duomai;
}
...
var Top = {
    Load: function () {
        var div_html = "";
        div_html += "<div class=\"wrap_top\">";
        div_html += "<p>";
        div_html += "<a href class=\"fl\">收藏</a>";
        div_html += "<a href>首页</a><a href class=\"on\">hi，15801658092</a><a href>退出</a><span>|</span><font>购物车<a href>0</a>件</font><a href>我的家有</a><font>可用优惠劵<a href>0</a>元</font><a href>我的收藏</a><span>|</span><label class=\"iphone\"><a href>手机版</a></label><label class=\"wx\"><a href>微信版</a></label>";
        div_html += "</p>";
        $("#div_top").html(div_html);
    },
};
//收藏夹
var FavoritesOperator = {
    Add: function () {
        try {
            ...
            if (document.all) {
                ...
                ShowMesaage('您的浏览器不支持,请按 Ctrl+D 手动收藏!');
            }
        } catch (e) {
            ShowMesaage('您的浏览器不支持,请按 Ctrl+D 手动收藏!');
        }
    ...
    RecordOrder: function () {
        try {
            ...
            if (localStorage[g_const_localStorage.OrderFrom] != "" && localStorage[g_const_localStorage.OrderFromRefer] != "") {
                ...
                //}
                ////测试代码
                //if (localStorage[g_const_localStorage.OrderFromRefer] == "") {
                //    alert("OrderFromRefer为空");
                //}
                //跳转支付页面
                OrderCreate.GoToPay(Merchant_duomai.order_sn);
...
//判断是多麦CPS否到达失效时间
if (!(localStorage[g_const_localStorage.OrderFrom] == undefined) && localStorage[g_const_localStorage.OrderFrom] == "duomai") {
    if (My_DateCheck.CheckEX("<=", localStorage[g_const_localStorage.Expires_duomai])) {
        localStorage[g_const_localStorage.OrderFrom] = "";
    }
}
