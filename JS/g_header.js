...
/*接口外部帮助地址*/var g_Help_APIUTL = "/JYH/API_Help.aspx";
/*接口外部帮助详情地址*/var g_HelpMsg_APIUTL = "/JYH/API_HelpMsg.aspx";
...
function SetLoginNoneInfo(tag, msg) {
    $("#message").hide();
...
function ShowMesaageCallback(sMessage, callback) {
    if (sMessage == "") {
        return;
    }
...
function ShowMesaage(sMessage, callBack) {
    if (sMessage == "") {
        return;
    }
    $("#sp_msg").html("<span></span>" + sMessage);
...
function renderTemplate(template, data) {
    var s = template.replace(g_const_regex_template, function (fullMatch, capture) {
        if (data[capture] || data[capture] == 0)
            return data[capture];
        else {
            return "";
        }
...
function g_GetBrandPictrue_S(picurl) {
    if (picurl == "")
        return g_brand_Pic;
    else
        return picurl;
